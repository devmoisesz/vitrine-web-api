...
const tagSchema = z.string().trim().min(1, 'Uma das tags está vazia.').max(30, 'Cada tag deve ter no máximo 30 caracteres.');
...
export const editProductBodySchema = z.object({
  newNameProduct: z.string().trim().min(1, 'Informe nome do produto.').optional(),
  newTags: z.array(tagSchema).max(10, 'É permitido informar no máximo 10 tags.').optional(),
  newDescription: z.string().trim().min(1, 'Informe a descrição do produto.').optional(),
  newPrice: z.number({ message: 'Informe preço do produto.' }).positive('O preço deve ser maior que R$ 0,00.').optional(),
  newStock: z.number({ message: 'Informe a quantidade de estoque do produto.' }).positive('A quantidade do estoque tem que ser positivo.').optional(),
  newCategory: z.string().trim().min(1, 'Informe uma categoria.').optional(),
  newSubcategory: z.string().trim().min(1, 'Informe uma subcategoria.').optional(),
});

export type RegisterProductBodySchema = z.infer<typeof registerProductBodySchema>;
