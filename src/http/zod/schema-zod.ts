...
export const registerCollaboratorBodySchema = z.object({
  name: z.string({ message: "O nome do colaborador é obrigatório" })
    .trim(),
...
export const registerStoreBodySchema = z.object({
  store_name: z.string({ message: "O nome da loja é obrigatório" })
    .trim(),
  ...
  owner_email: z.string({ message: "O e-mail do proprietário é obrigatório" })
    .email("Insira um e-mail de proprietário válido"),
    
  whatsapp: z.string({ message: "O WhatsApp é obrigatório" })
