...
describe('Uptade Product Status (E2E)', () => {
  ...
  test('[PATCH] /stores/:slug/products/:productId/status', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/stores/${store.slug}/products/${product.id}/status`)
    ...
    expect(productOnDatabase?.status).toEqual('INATIVO');
    expect(productOnDatabase?.status).not.toEqual('ATIVO');
  });
});
