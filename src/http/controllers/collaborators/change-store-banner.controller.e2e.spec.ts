...
describe('Change Store banner (E2E)', () => {
  ...
  test('[PATCH] /stores/:slug/banner/change', async () => {
    ...
    const banner = await prisma.store.findUnique({
      where: {
        id: store.id,
      },
    });
    ...
    const response = await request(app.getHttpServer())
      .patch(`/stores/${store.slug}/banner/change`)
      .set('Authorization', `Bearer ${accessToken}`)
    ...
    uploadedPublicId = imageOnDatabase.bannerPublicId;

    expect(imageOnDatabase).toBeTruthy();

    //verifica se a imagem antiga foi deletada
    expect(imageOnDatabase.bannerPublicId).not.toBe(banner.bannerPublicId);
  });
});
