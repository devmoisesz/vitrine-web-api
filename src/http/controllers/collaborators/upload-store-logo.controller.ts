...
@ApiBearerAuth()
@ApiTags('Upload Store Logo')
export class UploadStoreLogoController {
  constructor(private uploadStoreLogoService: UploadStoreLogoService) {}
  ...
  @ApiOperation({
    ...
    description: 'Uploads a logo image for a store.',
  })
    ...
    schema: {
      ...
      properties: {
        file: {
          ...
          format: 'binary',
          description:
            'Store logo image (png, jpg, jpeg or webp). Maximum 5MB.',
        },
      },
    },
  })
  ...
  @ApiConflictResponse({
    description: 'Store already has a logo.',
  })
  async handle(
    @UploadedFile(
      new ParseFilePipe({
