...
describe('Edit Selected Product (E2E)', () => {
  ...
  test('[PUT] /cart/:cartItemId', async () => {
    ...
    const cartItemOnDatabase = await prisma.cartItems.findFirst({
        where: {
            cartId: cart.id
        }
    })

    expect(cartItemOnDatabase).toBeTruthy()
    expect(cartItemOnDatabase?.selectedSize).toEqual("P")
    expect(cartItemOnDatabase?.quantity).toEqual(7)
  });
});
