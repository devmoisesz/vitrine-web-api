...
- GET `/me` — Obter perfil do usuário (Auth: `JwtAuthGuard`).
  Example response (200, `OutputGetProfileDto`):
  ```json
  {
  ...
  ```
- PUT `/account/edit` — Editar dados do usuário (Auth: `JwtAuthGuard`). Body: `{ name, email }`. Retorna status `204`.

## Endereços do usuário

- POST `/address/register` — Registrar endereço do usuário (Auth: `JwtAuthGuard`). Body: address fields (`label`, `cep`, `state`, `city`, `neighborhood`, `street`, `number`, `complement`). Retorna status `201`.
- GET `/me/addresses` — Listar endereços do usuário (Auth: `JwtAuthGuard`). Query: `?page=`.
  Example response (200, array of `Address`):
  ```json
  [
    {
      "id": "b1a2c3d4-...",
  ...
  ```
- PUT `/me/addressess/:addressId` — Atualizar endereço do usuário (Auth: `JwtAuthGuard`). Body: address fields. Retorna status `204`.
...
- GET `/store/:slug` — Obter perfil da loja (Auth: `JwtAuthGuard`).
  ...
  ```
- POST `/store` — Registrar loja (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Body: store data. Retorna status `201`.
- PATCH `/stores/:slug/deactivate` — Desativar loja (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Retorna status `204`.
- PATCH `/stores/:slug/activate` — Ativar loja (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Retorna status `204`.

## Dados da loja (colaborador)

- PUT `/store/:slug/edit` — Editar dados da loja (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Body: `{ newName, newEmail, newDescription }`. Retorna status `204`.
...
- GET `/products/:productId` — Lista apenas um produto, rota para interface para visualizar um produto (Public).
  Example response (200, `Product` with images):
  ```json
  {
    "product": {
...
- GET `/store/:slug/products` — Listar produtos por loja (Public). Query: `?name=&?categoryId=&?subcategoryId=&?page=`.
  Example response: mesma estrutura de `/products` (array de `Product`).
...
- POST `/stores/:slug/products` — Criar produto (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). Body: product data (`name_product`, `tags`, `description`, `price`, `sizes`, `stock`, `name_category`, `name_subcategory`).
  ...
  ```
- PUT `/stores/:slug/products/:productId` — Editar produto (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). Body: edited fields. Retorna status `204`.
- DELETE `/stores/:slug/products/:productId/` — Deletar produto (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). Retorna status `204`.
- PATCH `/stores/:slug/products/:productId/status` — Atualizar status do produto (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). Body: `{ status: "ATIVO"|"INATIVO" }`. Retorna status `204`.
...
- POST `/stores/:slug/productimages/:productId` — Upload de imagem (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). FormFile `file`, optional body `is_main`.
  Example response (201, `ProductImages` created):
  ```json
  {
    "id": "img-...",
    "productId": "p1-...",
...
- POST `/products/:productId/cart` — Adicionar produto no carrinho (Auth: `JwtAuthGuard`). Body: `{ quantity, size(opcional) }`. Retorna status `201`.
...
- GET `/cart/:cartId/products` — Listar produtos de um carrinho (Auth: `JwtAuthGuard`).
  ...
  ```
- PUT `/cart/:cartItemId` — Atualizar item registrado no carrinho (Auth: `JwtAuthGuard`). Body: `{ quantity, size }`. Retorna status `204`.
- DELETE `/cart/:cartItemId` — Remove item registrado do carrinho (Auth: `JwtAuthGuard`). Retorna status `204`.

## Pedidos

- POST `/cart/:cartId/order` — Registrar o pedido do usuário (Auth: `JwtAuthGuard`). Retorna status `201`.
- GET `/orders` — Listar pedidos do usuário (Auth: `JwtAuthGuard`). Query: `?page=`.
  Example response (200):
  ```json
  [
    {
  ...
  ```
- GET `/store/:slug/orders` — Listar pedidos de uma loja (Auth: `JwtAuthGuard` + `StoreAccessGuard`, roles: `FUNCIONARIO`|`PROPRIETARIO`). Query: `?page=`.
  Example response (200):
  ```json
  [
    {
...
- POST `/stores/:slug/logo` — Upload de logo (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). FormFile `file`. Retorna status `201`.
- PATCH `/stores/:slug/logo/change` — Alterar logo (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). FormFile `file`. Retorna status `200`.
- DELETE `/stores/:slug/logo/delete` — Deletar logo (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Retorna status `204`.

## Colaboradores / Funcionários

- POST `/stores/:storeId/collaborators` — Registrar colaborador (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Body: `{ name, email, password, role }`. Retorna status `201`.
- GET `/store/:slug/employees` — Listar colaboradores (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Query: `?page=`.
  Example response (200, array of employees):
  ```json
  [
    {
      "name": "Maria Silva",
  ...
  ```
- DELETE `/store/:slug/delete/:employeeId` — Remover colaborador (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Retorna status `204`.

## Endereços da loja

- POST `/address/:slug/register/` — Registrar endereço da loja (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Body: address fields (`label`, `cep`, `state`, `city`, `neighborhood`, `street`, `number`, `complement`). Retorna status `201`.
- PUT `/store/:slug/address` — Atualizar endereço da loja (Auth: `JwtAuthGuard` + `StoreAccessGuard`, role: `PROPRIETARIO`). Body: address fields (`cep`, `state`, `city`, `neighborhood`, `street`, `number`, `complement`). Retorna status `204`.

## Categorias / Subcategorias (Admin)

- POST `/categories` — Criar categoria (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Body: `{ name }`. Retorna status `201`.
- PUT `/categories/:slug/edit` — Editar categoria (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Body: `{ name }`. Retorna status `204`.
- POST `/categories/:slug/subcategory` — Criar subcategoria (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Body: `{ name }`. Retorna status `201`.
- PUT `/categories/:slug/subcategories/:id` — Editar subcategoria (Auth: `JwtAuthGuard` + `AdminAccessGuard`). Body: `{ name }`. Retorna status `204`.
- GET `/categories` — Listar categorias (Public).
  Example response (200, array of `Category`):
  ```json
  [
    {
      "id": "cat-...",
...
- GET `/subcategories` — Listar subcategorias (Public). Query: `?categoryId=`.
  Example response (200, array of `SubCategory`):
  ```json
  [
    {
      "id": "sub-...",
